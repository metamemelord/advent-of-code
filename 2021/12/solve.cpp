...
map<pair<int, pair<long long, bool>>, int> dp2;
...
int dfs(const vector<int> x[], int N, long long mask, const set<int> &uppers, int term, int start) {
    ...
    if(!contains(dp, make_pair(N, mask))) {
        int ans = 0;
        for (auto n: x[N]) {
            if(n == start) continue;
        ...
        dp[make_pair(N, mask)] = ans;
    }
    return dp[make_pair(N, mask)];
}

int dfs2(const vector<int> x[], int N, long long mask, bool smask, const set<int> &uppers, int term, int start) {
    if (N == term) return 1;
    
    const auto state = make_pair(N, make_pair(mask, smask));

    if (!contains(dp2, state)) {
        int ans = 0;
        for (auto n: x[N]) {
            if(n == start) continue;
            if(contains(uppers, N) && contains(uppers, n)) continue;
            ...
            if (!contains(uppers, n)) {
                if (mask & 1 << n && smask) continue;
                else if (mask & 1 << n) ans += dfs2(x, n, mask | 1 << n, !smask, uppers, term, start);
